...
contract Treasury is ContractGuard, Epoch, Orchestrator {
    ...
    uint256 public cashPriceCeiling2 = uint256(95).mul(cashPriceOne).div(10**2);
    ...
    uint256 public supplyDeltaRatio = 80;
    uint256 public fundAllocationRatio = 10;
    uint256 public treasuryReserveRatio = 10;
    uint256 public boardroomRatio = 0;
    uint256 public deflationRatio = 15;
        ...
        checkOperator
    {
        ...
        // > 0.95 < 1.05
        if (cashPrice >= cashPriceCeiling2 && cashPrice <= cashPriceCeiling) {
        ...
        // circulating supply
        inflationDeflationAmount = computeSupplyDelta(cashPrice,cashPriceOne,IERC20(cash).totalSupply());
        
        int256 deflation = uint256(inflationDeflationAmount).mul(deflationRatio).div(100).toInt256Safe();
        // Price is less than 0.95 deflation
        if (cashPrice < cashPriceCeiling2) {
            _carrotRebase(epoch,deflation);
        ...
        uint256 orientation = uint256(computeSupplyDelta(cashPrice,cashPriceOne,IERC20(cash).totalSupply().sub(IERC20(cash).balanceOf(address(this)))));
        ...
        if (IERC20(veCarrot).totalSupply() > accumulatedSeigniorage){
            uint256 treasuryReserve = orientationInflation.mul(treasuryReserveRatio).div(100);
    ...
    function withdrawalToken(address _tokenAddress,uint256 _value,address _to) onlyOperator public { 
